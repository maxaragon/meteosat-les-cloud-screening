...
def _determine_region_info(lat: float, lon: float) -> dict:
    """Dynamically determine map extent and region name using geographic boundaries."""
    geojson_path = Path(__file__).parent / "countries.geojson"
    if not geojson_path.exists():
        logging.warning(f"countries.geojson not found in script directory, using a default 10x10 degree box.")
        return {"lon": [lon - 5, lon + 5], "lat": [lat - 5, lat + 5], "name": "Region"}
    ...
    try:
        ...
        for idx, row in gdf.iterrows():
            if row.geometry.contains(point):
                ...
                # Use a focused region around the site, but slightly larger than the AOI
                padding = 2.5 # degrees padding
                lon_extent = [lon - padding, lon + padding]
                lat_extent = [lat - padding, lat + padding]
                
                logging.info(f"Site located in {country_name}. Setting map extent around site.")
                return {"lon": lon_extent, "lat": lat_extent, "name": country_name}
        
        logging.warning(f"No country found for coordinates ({lat}, {lon}). Using a default 10x10 degree box.")
        return {"lon": [lon - 5, lon + 5], "lat": [lat - 5, lat + 5], "name": "Region"}

    except Exception as e:
        logging.error(f"Error reading GeoJSON file: {e}. Using a default 10x10 degree box.")
        return {"lon": [lon - 5, lon + 5], "lat": [lat - 5, lat + 5], "name": "Region"}
...
class TopDayVisualizer:
    ...
    def _plot_frame(self, ts: datetime, ct_region: np.ndarray, ct_aoi: np.ndarray, pct_aoi: dict,
        ...
        im = ax1.pcolormesh(self._lon2d[self._region_slice], self._lat2d[self._region_slice], ct_region,
        ...
        ax2.pcolormesh(aoi_lon, aoi_lat, ct_aoi, cmap=self.cmap, vmin=-0.5, vmax=2.5, shading='auto')
...
def main():
    ...
    site = os.environ.get('SITE', 'PALAISEAU') # Default to PALAISEAU if not set
